...
function HealthBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full border ${colors[status] || 'bg-slate-500/15 text-slate-400 border-slate-500/30'}`}>
      {status || 'Unknown'}
    </span>
  );
}
...
function ScoreRing({ score, size = 48, stroke = 4 }: { score: number; size?: number; stroke?: number }) {
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center text-sm font-bold text-slate-200">{score}</div>
    </div>
  );
}
    ...
    <PageShell
      ...
      actions={
        <div className="flex flex-wrap gap-2">
          ...
          <button className="btn-primary" type="button" onClick={() => downloadJson('workos-summary.json', data)}>Download</button>
        </div>
      }
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="card flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm font-semibold text-slate-200">{data?.priority?.level || '—'}</div>
          </div>
        </div>
        <div className="card flex items-center gap-3">
          <ScoreRing score={data?.risk?.score ?? 0} />
        ...
        <div className="card flex items-center gap-3">
          <div className="relative w-12 h-12">
            <svg width={48} height={48} className="-rotate-90">
              ...
              <circle cx={24} cy={24} r={20} fill="none" stroke="currentColor" strokeWidth={4} strokeDasharray={125.66} strokeDashoffset={125.66 - ((data?.workload?.capacity_utilization ?? 0) / 100) * 125.66} strokeLinecap="round" className="text-primary" />
            </svg>
            <div className="absolute inset-0 flex items-center justify-center text-xs font-bold text-slate-200">{data?.workload?.capacity_utilization ?? 0}%</div>
          </div>
          <div>
            <div className="text-xs uppercase tracking-wide text-slate-400">Workload</div>
            <div className={`text-sm font-semibold ${data?.workload?.overloaded ? 'text-amber-400' : 'text-emerald-400'}`}>
      ...
      <div className="card bg-gradient-to-r from-primary/10 to-transparent border-primary/20">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex-1">
            <div className="text-sm font-medium text-slate-200">{data?.summary || 'No summary available.'}</div>
              ...
              <ul className="space-y-2">
                {data.next_best_actions.map((a: string, idx: number) => (
                  <li key={idx} className="flex gap-3 text-sm">
                    <span className="shrink-0 w-5 h-5 rounded-full bg-primary/20 text-primary flex items-center justify-center text-[10px] font-bold">{idx + 1}</span>
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {(data?.risk?.risks || []).length > 0 && (
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-rose-400 font-semibold mb-1.5">Risks</div>
              ...
              {(data?.risk?.mitigations || []).length > 0 && (
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-emerald-400 font-semibold mb-1.5">Mitigations</div>
                  <ul className="space-y-1">
                ...
                {topTasks.map((t: any, idx: number) => (
                  <div key={t._id} className="px-4 py-3 hover:bg-slate-700/30 transition-colors">
                    <div className="flex items-start gap-3">
                      <div className="shrink-0 w-6 h-6 rounded-full bg-primary/20 text-primary flex items-center justify-center text-[10px] font-bold mt-0.5">{idx + 1}</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <Link to={`/admin/task/${t._id}`} className="text-sm font-medium text-slate-200 hover:text-primary truncate">{t.title}</Link>
                          {t.blocked && <span className="shrink-0 text-[9px] px-1.5 py-0.5 rounded bg-rose-500/15 text-rose-400 font-semibold">BLOCKED</span>}
                        </div>
                        <div className="flex flex-wrap items-center gap-2 mt-1.5">
                          <span className={`inline-flex px-1.5 py-0.5 text-[10px] font-semibold rounded ${getPriorityColor(t.status === 'Completed' ? 'Low' : t.risk?.score > 50 ? 'High' : t.risk?.score > 25 ? 'Medium' : 'Low')}`}>
                            {t.status}
                          </span>
                          <span className="text-[10px] text-slate-500">Score: {t.priority?.score ?? 0}</span>
                          <span className="text-[10px] text-slate-500">Impact: {t.impactScore ?? '—'}/10</span>
                          <span className="text-[10px] text-slate-500">Effort: {t.effortHours ?? '—'}h</span>
                          {t.assignedTo?.name && <span className="text-[10px] text-slate-500">→ {t.assignedTo.name}</span>}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {blockedTasks.length > 0 && (
            <div className="card !p-0 overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-700">
                <h3 className="text-sm font-semibold text-rose-400">Blocked Tasks ({blockedTasks.length})</h3>
              </div>
              <div className="divide-y divide-slate-700/50">
                {blockedTasks.map((t: any) => (
                  <div key={t._id} className="px-4 py-3">
                    <Link to={`/admin/task/${t._id}`} className="text-sm font-medium text-slate-200 hover:text-primary">{t.title}</Link>
      ...
      {activeTab === 'schedule' && (
        <div className="card !p-0 overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-700">
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Deep-work blocks for top priority tasks</p>
          </div>
            ...
            <div className="divide-y divide-slate-700/50">
              {data.suggested_schedule.map((s: any, idx: number) => (
                <div key={idx} className="px-4 py-3 flex items-start gap-3">
                  <div className="shrink-0 w-8 h-8 rounded-lg bg-primary/10 text-primary flex items-center justify-center text-xs font-bold">{idx + 1}</div>
                  <div className="flex-1">
                    <Link to={`/admin/task/${s.taskId}`} className="text-sm font-medium text-slate-200 hover:text-primary">
      ...
      {activeTab === 'insights' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="card">
            <h3 className="text-sm font-semibold text-slate-200 mb-3 flex items-center gap-2">
              <svg className="w-4 h-4 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
              Active Automations
            </h3>
            <ul className="space-y-2">
          ...
          <div className="card">
            ...
            <div className="flex items-center gap-4 mb-3">
              <div className="text-3xl font-bold text-slate-200">{data?.goal_alignment?.score ?? 0}%</div>
              <div className="text-xs text-slate-400">Strategic alignment score</div>
            </div>
            {(data?.goal_alignment?.related_goals || []).length > 0 && (
              <div className="text-xs text-slate-400">{data.goal_alignment.related_goals.length} goals linked</div>
            )}
          </div>
                ...
                {data.critical_path.map((taskId: string, idx: number) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-2">
                      <Link to={`/admin/task/${taskId}`} className="text-xs px-2.5 py-1.5 rounded-lg bg-slate-800 border border-slate-700 text-slate-300 hover:border-primary/50 hover:text-primary transition-colors">
                        {task?.title || taskId}
                      </Link>
                      {idx < data.critical_path.length - 1 && <span className="text-slate-600">→</span>}
                    </div>
                  );
                })}
              </div>
