        ...
        <PageShell
            ...
            actions={
                <div className="flex gap-2">
                    <Link to="/admin/workos" className="btn-secondary">WorkOS</Link>
                    <Link to="/admin/create-task" className="btn-primary">Create Task</Link>
                </div>
            }
        >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div className="lg:col-span-2 card">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            <Link to="/admin/manage-tasks" className="text-[10px] text-primary hover:text-primary-hover font-medium">View all →</Link>
                        </div>
                        {urgentTasks.length === 0 ? (
                            ...
                            <div className="divide-y divide-slate-700/50">
                                {urgentTasks.map(t => {
                                    ...
                                    return (
                                        <Link key={t._id} to={`/admin/task/${t._id}`} className="flex items-center gap-3 py-2.5 px-2 -mx-2 rounded-lg hover:bg-slate-700/30 transition-colors group">
                    ...
                    <div className="card">
                        ...
                        <div className="space-y-2">
                            <Link to="/admin/create-task" className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-700 hover:border-primary/40 hover:bg-slate-700/30 transition-all group">
                                ...
                                <div className="flex-1">
                                    <div className="text-sm font-medium text-slate-200 group-hover:text-primary transition-colors">Create Task</div>
                                    <div className="text-[10px] text-slate-500">Assign to team members</div>
                                </div>
                            </Link>
                            <Link to="/admin/manage-tasks" className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-700 hover:border-blue-400/40 hover:bg-slate-700/30 transition-all group">
                                ...
                                <div className="flex-1">
                                    <div className="text-sm font-medium text-slate-200 group-hover:text-primary transition-colors">Manage Tasks</div>
                                    <div className="text-[10px] text-slate-500">View and update all tasks</div>
                                </div>
                            </Link>
                            <Link to="/admin/projects" className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-700 hover:border-violet-400/40 hover:bg-slate-700/30 transition-all group">
                                ...
                                <div className="flex-1">
                                    <div className="text-sm font-medium text-slate-200 group-hover:text-primary transition-colors">Projects</div>
                                    <div className="text-[10px] text-slate-500">Manage project outcomes</div>
                                </div>
                            </Link>
                            <Link to="/admin/manage-users" className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-700 hover:border-emerald-400/40 hover:bg-slate-700/30 transition-all group">
                                ...
                                <div className="flex-1">
                                    <div className="text-sm font-medium text-slate-200 group-hover:text-primary transition-colors">Team</div>
                    ...
                    {dashboardData?.recentTasks?.length === 0 ? (
                        ...
                        <div className="divide-y divide-slate-700/50">
                            {dashboardData?.recentTasks?.slice(0, 8).map(task => {
                                ...
                                return (
                                    <Link key={task._id} to={`/admin/task/${task._id}`} className="flex items-center gap-3 py-2.5 px-2 -mx-2 rounded-lg hover:bg-slate-700/30 transition-colors group">
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                ...
                                                {task.dueDate && (
                                                    <span className={`text-[10px] ${overdue ? 'text-rose-400 font-medium' : 'text-slate-500'}`}>
                                                        {overdue ? `${Math.abs(daysLeft!)}d overdue` : daysLeft === 0 ? 'Today' : `${daysLeft}d left`}
                                                    </span>
                                                )}
                                            </div>
