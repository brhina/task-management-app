...
const TIMEFRAMES: { value: GoalTimeframe; label: string; desc: string; color: string; icon: string }[] = [
...
<div className='grid grid-cols-1 lg:grid-cols-2 gap-3'>
        {/* Goal Title */}
        <div className="card">
          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Goal Title *</div>
          <input
            type="text"
            value={title}
        ...
        {/* Objective */}
        <div className="card">
          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Objective</div>
          <textarea
            value={objective}
        ...
        {/* Timeframe Selector */}
        <div className="card">
          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Timeframe</div>
        ...
        {/* Metric & Target */}
        <div className="card">
          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Measurement</div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-1">Target Value</div>
              <input
                type="number"
                min={0}
                value={targetValue}
          ...
          {metric && targetValue !== '' && (
            <div className="mt-3 p-3 rounded-lg bg-slate-800/50 border border-slate-700/50">
              ...
              <div className="text-sm text-slate-200 mt-1">
                Achieve <span className="font-semibold text-primary">{metric}</span> = <span className="font-semibold text-primary">{targetValue}</span> within <span className="font-semibold">{timeframe.toLowerCase()}</span> timeframe
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end gap-2">
          <Link to="/admin/goals" className="btn-secondary">Cancel</Link>
          <button type="submit" disabled={creating} className="btn-primary disabled:opacity-50 min-w-[140px]">
            {creating ? (
