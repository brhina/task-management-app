...
function UserWorkOS() {
    ...
    const productivityScore = useMemo(() => {
        ...
        const currentScore = (stats.completed * weights.completed) + (stats.inProgress * weights.inProgress) + (stats.inReview * weights.inReview);
    ...
    const weeklyProgress = useMemo(() => {
        ...
        const completedThisWeek = tasks.filter(t => t.status === 'Completed' && new Date(t.updatedAt) >= weekAgo).length;
        ...
        <PageShell
            ...
            actions={
                <div className="flex gap-2">
                    ...
                    <Link to="/user/my-tasks" className="btn-primary">My Tasks</Link>
                </div>
            }
        >
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <div className="card flex items-center gap-3">
                        ...
                        <div>
                            <div className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold">Productivity</div>
                            <div className={`text-sm font-bold ${productivityScore >= 70 ? 'text-emerald-400' : productivityScore >= 40 ? 'text-amber-400' : 'text-rose-400'}`}>
                                {productivityScore >= 70 ? 'Strong' : productivityScore >= 40 ? 'Moderate' : 'Needs Focus'}
                            </div>
                        </div>
                    </div>
                ...
                <div className="card">
                    <div className="flex items-center justify-between mb-2">
                        <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide">This Week</div>
                        <span className="text-xs text-slate-500">{weeklyProgress.completedThisWeek} tasks completed</span>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="card">
                            ...
                            {urgentTasks.length === 0 ? (
                                <div className="text-center py-6">
                                    <svg className="w-8 h-8 text-emerald-500/50 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="card">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <Link to="/user/my-tasks?priority=High" className="text-[10px] text-primary hover:text-primary-hover font-medium">View all →</Link>
                            </div>
                                        ...
                                        <Link key={t._id} to={`/user/task/${t._id}`} className="flex items-center gap-3 py-2.5 px-2 -mx-2 rounded-lg hover:bg-slate-700/30 transition-colors group">
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    <span className={`inline-flex px-1.5 py-0.5 text-[10px] font-semibold rounded ${getPriorityColor(t.priority)}`}>{t.priority}</span>
                                                    {t.dueDate && (
                                                        <span className="text-[10px] text-slate-500">Due {new Date(t.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                                                    )}
                                                </div>
                                            </div>
                    ...
                    <div className="space-y-4">
                        {/* Task Breakdown */}
                        <div className="card">
                            <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Task Breakdown</div>
                            <div className="space-y-2">
                                {[
                                    ...
                                    <div key={item.label} className="flex items-center justify-between">
                                        ...
                                        <span className={`text-xs font-bold tabular-nums ${item.textColor}`}>{item.count}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="card">
                            <div className="flex items-center justify-between mb-3">
                                <div className="text-xs font-semibold text-emerald-400 uppercase tracking-wide">Recently Done</div>
                                <Link to="/user/my-tasks?status=completed" className="text-[10px] text-primary hover:text-primary-hover font-medium">View all →</Link>
                            </div>
                            {recentCompleted.length === 0 ? (
                                <div className="text-xs text-slate-500 text-center py-4">No completed tasks yet.</div>
                            ) : (
                                <div className="space-y-2">
                                    {recentCompleted.map(t => (
                                        <Link key={t._id} to={`/user/task/${t._id}`} className="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-700/30 transition-colors group">
                                            <svg className="w-4 h-4 text-emerald-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                            </svg>
                                            <div className="min-w-0 flex-1">
                                                <div className="text-xs font-medium text-slate-300 group-hover:text-primary truncate transition-colors">{t.title}</div>
                                                <div className="text-[10px] text-slate-500">{getRelativeTime(t.updatedAt)}</div>
                                            </div>
                                        </Link>
                                    ))}
                                </div>
                        ...
                        {/* Quick Stats */}
                        <div className="card">
                            <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Insights</div>
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className={`text-xs font-bold ${stats.overdue > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>{stats.overdue}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-xs text-slate-500">Avg. tasks/day</span>
                                    <span className="text-xs font-bold text-slate-200">{stats.total > 0 ? Math.round(stats.total / 7) : 0}</span>
                                </div>
                            </div>
                        </div>
                    </div>
