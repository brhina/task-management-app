...
const STATUS_OPTIONS = [
    ...
    { value: TASK_STATUS.PENDING, label: 'Pending', color: 'bg-yellow-500/15 text-yellow-400 border-yellow-500/30' },
    { value: TASK_STATUS.IN_PROGRESS, label: 'In Progress', color: 'bg-blue-500/15 text-blue-400 border-blue-500/30' },
    { value: TASK_STATUS.IN_REVIEW, label: 'In Review', color: 'bg-purple-500/15 text-purple-400 border-purple-500/30' },
    { value: TASK_STATUS.COMPLETED, label: 'Completed', color: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' },
];
                        ...
                        <div className="flex items-center bg-slate-800 border border-slate-700 rounded-lg p-1">
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
                                    viewMode === 'board' ? 'bg-primary text-white' : 'text-slate-400 hover:text-slate-200'
                                }`}
                            >
                    ...
                    <div className="card !p-0 overflow-hidden">
                        <div className="divide-y divide-slate-700/50">
                            {filteredTasks.map(task => {
                                ...
                                return (
                                    <div key={task._id} className="px-2 py-3 hover:bg-slate-700/30 transition-colors group">
                                        <div className="flex items-center gap-3">
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <Link to={`/user/task/${task._id}`} className="text-sm font-medium text-slate-200 group-hover:text-primary truncate transition-colors">
                                                        {task.title}
                                                    </Link>
                                                ...
                                                <div className="flex items-center gap-3 text-[10px] text-slate-500">
                                                    ...
                                                    {task.projectId && (
                                                        <span className="text-slate-600">
                                                            {projects.find(p => p._id === (typeof task.projectId === 'string' ? task.projectId : task.projectId?._id))?.name || 'Project'}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <Link to={`/user/task/${task._id}`} className="text-[10px] text-primary hover:text-primary-hover font-medium shrink-0">
                                                View →
                                            </Link>
                                        </div>
                                    </div>
