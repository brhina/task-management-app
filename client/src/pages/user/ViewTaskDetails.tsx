...
const STATUS_FLOW = [
    { value: TASK_STATUS.PENDING, label: 'Pending', color: 'bg-yellow-500/15 text-yellow-400 border-yellow-500/30 hover:bg-yellow-500/25' },
    { value: TASK_STATUS.IN_PROGRESS, label: 'In Progress', color: 'bg-blue-500/15 text-blue-400 border-blue-500/30 hover:bg-blue-500/25' },
    { value: TASK_STATUS.IN_REVIEW, label: 'In Review', color: 'bg-purple-500/15 text-purple-400 border-purple-500/30 hover:bg-purple-500/25' },
    { value: TASK_STATUS.COMPLETED, label: 'Completed', color: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/25' },
];
...
function ViewTaskDetails() {
    ...
    const handleStatusUpdate = async (newStatus: string) => {
        try {
            setUpdating(true);
            setError('');
            await api.put(apiPaths.TASKS.UPDATE_TASK_STATUS.replace(':id', id || ''), { status: newStatus });
    ...
    const handleChecklistUpdate = async (todoCheckList: TodoItem[]) => {
        try {
            setUpdating(true);
            setError('');
            await api.put(apiPaths.TASKS.UPDATE_TASK_CHECKLIST.replace(':id', id || ''), { todoCheckList });
            await fetchTaskDetails();
        } catch (err: any) {
    ...
    const handleAddPrereq = async () => {
        ...
        try {
            setAddingDep(true);
            await api.post(apiPaths.DEPENDENCIES.CREATE, { fromTaskId: prereqToAdd, toTaskId: task._id, type: 'FS' });
            setPrereqToAdd('');
    ...
    if (!task) {
        return (
            <PageShell title="Not Found" subtitle={error || 'Task not found.'}>
                <Link to={isAdminRoute ? "/admin/manage-tasks" : "/user/my-tasks"} className="btn-primary">Back to Tasks</Link>
            </PageShell>
        );
    }

    const blockedBy = dependencies.filter(d => String((d.toTaskId as any)?._id || d.toTaskId) === task._id);
    const blocking = dependencies.filter(d => String((d.fromTaskId as any)?._id || d.fromTaskId) === task._id);
        ...
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="card">
                        <div className="flex items-center justify-between mb-3">
                            ...
                            {totalCount > 0 && (
                                <span className="text-xs text-slate-400 tabular-nums">{completedCount}/{totalCount}</span>
                            )}
                        </div>
                        ...
                        {totalCount > 0 && (
                            <div className="mb-3">
                                ...
                                <div className="text-[10px] text-slate-500 mt-1 text-right">{checklistProgress}%</div>
                            </div>
                        )}

                        <div className="space-y-1.5">
                            {task.todoCheckList?.map((todo, index) => (
                                <div key={index} className="flex items-center gap-2.5 group px-2 py-1.5 rounded-lg hover:bg-slate-700/30 transition-colors">
                                    <input
                                        type="checkbox"
                                    ...
                                    />
                                    <span className={`flex-1 text-sm ${todo.isCompleted ? 'line-through text-slate-500' : 'text-slate-300'}`}>
                                        {todo.text}
                                    </span>
                                    <button
                        ...
                        {showAddChecklist ? (
                            <form onSubmit={handleAddChecklistItem} className="mt-3 flex gap-2">
                                <input
                                    ...
                                    autoFocus
                                />
                                <button type="submit" disabled={!newChecklistItem.trim() || updating} className="btn-primary px-3 py-1.5 text-sm disabled:opacity-50">
                                    Add
                                </button>
                                <button type="button" onClick={() => { setShowAddChecklist(false); setNewChecklistItem(''); }} className="btn-secondary px-3 py-1.5 text-sm">
                                    Cancel
                                </button>
                            </form>
                        ) : (
                    ...
                    {/* Dependencies */}
                    <div className="card">
                        <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Dependencies</div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-1.5">Blocked By</div>
                                {blockedBy.length === 0 ? (
                                    <div className="text-xs text-slate-500 italic">None</div>
                                ) : (
                                    <div className="space-y-1.5">
                                        {blockedBy.map((d: any) => (
                                            <div key={d._id} className="flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-lg border border-slate-700 bg-slate-800/50">
                                                <div className="min-w-0">
                                                    <div className="text-xs text-slate-300 truncate">{d.fromTaskId?.title || 'Unknown'}</div>
                                                    <div className="text-[10px] text-slate-500">{d.fromTaskId?.status} • {d.type}</div>
                                                </div>
                                                {user?.role === 'Admin' && (
                                                    <button onClick={() => handleRemoveDependency(d._id)} className="text-rose-400 hover:text-rose-300 text-xs shrink-0">✕</button>
                                                )}
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div>
                                <div className="text-[10px] uppercase tracking-wider text-slate-500 mb-1.5">Blocking</div>
                                {blocking.length === 0 ? (
                                    <div className="text-xs text-slate-500 italic">None</div>
                                ) : (
                                    <div className="space-y-1.5">
                                        {blocking.map((d: any) => (
                                            <div key={d._id} className="flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-lg border border-slate-700 bg-slate-800/50">
                                                <div className="min-w-0">
                                                    <div className="text-xs text-slate-300 truncate">{d.toTaskId?.title || 'Unknown'}</div>
                                                    <div className="text-[10px] text-slate-500">{d.toTaskId?.status} • {d.type}</div>
                                                </div>
                                                {user?.role === 'Admin' && (
                                                    <button onClick={() => handleRemoveDependency(d._id)} className="text-rose-400 hover:text-rose-300 text-xs shrink-0">✕</button>
                                                )}
                        ...
                        {user?.role === 'Admin' && (
                            <div className="mt-3 pt-3 border-t border-slate-700/50">
                                <div className="flex gap-2">
                                    <select className="input-dark flex-1 text-sm" value={prereqToAdd} onChange={(e) => setPrereqToAdd(e.target.value)}>
                                        <option value="">Add prerequisite...</option>
                                        {tasksForDeps.filter(t => t._id !== task._id).slice(0, 200).map((t) => (
                                            <option key={t._id} value={t._id}>{t.title}</option>
                                        ))}
                                    </select>
                                    <button className="btn-primary px-3 py-1.5 text-sm disabled:opacity-50" disabled={!prereqToAdd || addingDep} onClick={handleAddPrereq}>
                                        {addingDep ? '...' : 'Add'}
                                    </button>
                                </div>
                            </div>
                ...
                <div className="space-y-4">
                    {/* Status Flow */}
                    <div className="card">
                        <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Status</div>
                        <div className="space-y-1.5">
                            {STATUS_FLOW.map((s, idx) => (
                                ...
                                >
                                    <div className={`w-2 h-2 rounded-full ${
                                        idx < currentStatusIdx ? 'bg-emerald-500' : idx === currentStatusIdx ? 'bg-primary' : 'bg-slate-600'
                                    }`} />
                                    {s.label}
                                    {task.status === s.value && (
                    ...
                    {/* Progress */}
                    <div className="card">
                        <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Progress</div>
                        <div className="flex items-center gap-3">
                            ...
                            />
                            <span className="text-sm font-bold text-slate-200 tabular-nums w-10 text-right">{task.progress || 0}%</span>
                        </div>
                    </div>
                    ...
                    {task.assignedTo && (
                        <div className="card">
                            <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Assignee</div>
                            {typeof task.assignedTo === 'object' ? (
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                        <div className="text-sm font-medium text-slate-200 truncate">{task.assignedTo.name || 'Unknown'}</div>
                    ...
                    {/* Dates */}
                    <div className="card">
                        <div className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Dates</div>
                        <div className="space-y-2.5">
                            <div className="flex items-center justify-between">
                                <span className="text-xs text-slate-500">Due</span>
                                <span className={`text-xs font-medium ${isOverdue(task.dueDate) && task.status !== TASK_STATUS.COMPLETED ? 'text-rose-400' : 'text-slate-300'}`}>
                            ...
                            {daysUntilDue !== null && (
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className={`text-xs font-medium ${
                                        isOverdue(task.dueDate) ? 'text-rose-400' :
                                        daysUntilDue <= 3 ? 'text-amber-400' : 'text-slate-300'
                                    }`}>
