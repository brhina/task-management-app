...
import AutomationRule, { type AutomationTrigger } from "../models/AutomationRule.js";
...
function matchesConditions(rule: any, task: any): boolean {
    ...
    if (c.projectId && String(task.projectId || "") !== String(c.projectId)) return false;
    if (c.tagIncludes) {
...
async function runActions(params: {
    ...
    for (const action of params.rule.actions || []) {
        ...
        if (type === "create_dependent_task" && params.task) {
            ...
            const descriptionTemplate = String(action.description || "Auto-created from {{title}} completion.");
            const title = titleTemplate.replace(/\{\{title\}\}/g, params.task.title);
            const description = descriptionTemplate.replace(/\{\{title\}\}/g, params.task.title);
...
export async function runAutomations(params: {
    ...
    const rules = await AutomationRule.find({ orgId: params.orgId, enabled: true, trigger: params.trigger }).sort({ createdAt: 1 });
    for (const rule of rules) {
        if (params.task && !matchesConditions(rule, params.task)) continue;
        await runActions({ orgId: params.orgId, trigger: params.trigger, task: params.task, rule });
    }
}
