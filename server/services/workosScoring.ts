...
export function urgencyScore(task: Pick<ITaskDocument, "dueDate" | "status">): number {
...
export function delayTolerancePenalty(priority: "Low" | "Medium" | "High"): number {
...
export function healthFromRiskAndOverdue(params: { riskScore: number; overdueCount: number; blockedCount: number }): HealthStatus {
    if (params.overdueCount > 0 && params.riskScore >= 70) return "critical";
    if (params.overdueCount > 0 || params.blockedCount > 5 || params.riskScore >= 70) return "delayed";
