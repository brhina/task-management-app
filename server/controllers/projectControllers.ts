...
export const listProjects = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const projects = await Project.find({ orgId: req.orgId }).sort({ createdAt: -1 });
...
export const getProjectById = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        res.status(200).json({ message: "Project fetched successfully", data: { project, tasks } });
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
};
...
export const createProject = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const { name, description, status, startDate, targetDate, ownerId } = req.body;
...
export const deleteProject = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        await Task.updateMany({ orgId: req.orgId, projectId: project._id }, { $unset: { projectId: "" } });
