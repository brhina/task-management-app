...
export const listAutomationRules = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const rules = await AutomationRule.find({ orgId: req.orgId }).sort({ createdAt: -1 });
        res.status(200).json({ message: "Automation rules fetched successfully", data: rules });
    } catch (error: any) {
...
export const createAutomationRule = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        res.status(201).json({ message: "Automation rule created successfully", data: rule });
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
};
...
export const updateAutomationRule = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const rule = await AutomationRule.findOne({ _id: req.params.id, orgId: req.orgId });
        if (!rule) {
        ...
        res.status(200).json({ message: "Automation rule updated successfully", data: updated });
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
};
...
export const runDailySummaryJob = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!req.orgId) {
            res.status(400).json({ message: "Organization context is required" });
            return;
