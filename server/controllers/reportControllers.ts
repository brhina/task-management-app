...
const exportTasksReport = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!req.orgId) {
        ...
        const tasks = await Task.find({ orgId: req.orgId }).populate('assignedTo', 'name email');
...
const exportUsersReport = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!req.orgId) {
            res.status(400).json({ message: 'Organization context is required' });
            return;
        ...
        const memberships = await OrgMembership.find({ orgId: req.orgId, status: 'Active' });
        const userIds = memberships.map((m: any) => m.userId);
        const users = await User.find({ _id: { $in: userIds } }).select('name email _id createdAt');
        const userTasks = await Task.find({ orgId: req.orgId }).populate('assignedTo', 'name email _id createdAt');
