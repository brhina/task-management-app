...
export const listDependencies = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!req.orgId) {
        ...
        res.status(200).json({ message: "Dependencies fetched successfully", data: deps });
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
};

export const createDependency = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!req.orgId) {
            res.status(400).json({ message: "Organization context is required" });
            return;
        ...
        if (!fromTask || !toTask) {
            res.status(404).json({ message: "Both tasks must exist in the same organization" });
            return;
        }
        ...
        res.status(201).json({ message: "Dependency created successfully", data: dep });
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
};

export const deleteDependency = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!req.orgId) {
            res.status(400).json({ message: "Organization context is required" });
            return;
        ...
        const dep = await Dependency.findOne({ _id: req.params.id, orgId: req.orgId });
        if (!dep) {
...
export const getDependencyAnalysis = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        if (!req.orgId) {
            res.status(400).json({ message: "Organization context is required" });
            return;
        ...
        res.status(200).json({ message: "Dependency analysis computed", data: analysis });
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
};
