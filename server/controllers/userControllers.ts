...
export const getUserById = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const membership = await OrgMembership.findOne({ orgId: req.orgId, userId: req.params.id, status: 'Active' });
        if (!membership) {
...
export const deleteUser = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const membership = await OrgMembership.findOne({ orgId: req.orgId, userId: id });
        if (!membership) {
        ...
        const remainingMemberships = await OrgMembership.countDocuments({ userId: id });
