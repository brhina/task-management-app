    ...
    if (membership.role === "OrgAdmin") {
      ...
      if (adminCount <= 1) {
        res
          ...
          .json({
            message:
              "Cannot leave organization as the last admin. Transfer admin role first.",
          });
        return;
      }
    }
...
export const generateInviteToken = async (
  ...
  try {
    ...
    if (!membership) {
      res
        .status(403)
        .json({ message: "Only org admins can generate invite tokens" });
      return;
    }
...
export const updateMemberRole = async (
  ...
  try {
    ...
    if (!adminMembership) {
      res
        .status(403)
        .json({ message: "Only org admins can update member roles" });
      return;
    }
...
export const removeMember = async (
  ...
  try {
    ...
    if (String(membership.userId) === String(req.user._id)) {
      res
        .status(400)
        .json({ message: "Cannot remove yourself from the organization" });
      return;
    }
